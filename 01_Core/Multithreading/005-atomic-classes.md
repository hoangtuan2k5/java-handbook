...
- Atomic class dùng cho counter, flag, single reference, hoặc CAS loop nhỏ.
- Atomic của một field không làm cả object thread-safe.
- `get()` rồi `set()` riêng không phải atomic có điều kiện; dùng `compareAndSet`/`updateAndGet`.
- CAS loop nặng contention có thể retry nhiều; cân nhắc lock khi quá phức tạp.
- Khi cần nhiều field nhất quán, lock thường rõ hơn atomic chain.
