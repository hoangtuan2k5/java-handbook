...
- `CompletableFuture` dùng để compose async; tránh dùng chỉ để bọc một blocking call rồi `join()` ngay.
- Blocking I/O phải chạy trên executor riêng, không dùng common ForkJoinPool.
- `thenCompose` cho async tiếp theo; `thenApply` cho transform sync; không trộn lẫn.
- Error path phải có `exceptionally`/`handle`; không bỏ exception chìm.
- Không gọi `.get()` trong handler chain; ưu tiên non-blocking compose.
