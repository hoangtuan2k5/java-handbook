...
if (Skip current item?) then (yes)
  :continue;
  :Next iteration;
  stop
else (no)
  if (Stop loop only?) then (yes)
    :break;
    :Code after loop;
    stop
  else (no)
    if (Method is done?) then (yes)
      :return;
      stop
    else (no)
      :Finish iteration normally;
      stop
    endif
  endif
endif
...
note right
continue tác động iteration hiện tại.
break tác động loop gần nhất.
return tác động toàn bộ method.
end note
@enduml
```
