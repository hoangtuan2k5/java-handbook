...
- `CountDownLatch` cho one-shot wait; sau khi count về 0 không reset.
- `CyclicBarrier` cho nhiều phase đồng bộ với cùng nhóm worker.
- `await()` là blocking; tránh chạy trên request thread hoặc pool nhỏ.
- Coordination không thay thế lock; shared state vẫn cần thread-safety riêng.
- Thiếu participant ở `CyclicBarrier` làm cả nhóm treo; phải có timeout/abort.
