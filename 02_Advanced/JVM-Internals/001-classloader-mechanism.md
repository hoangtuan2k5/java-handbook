...
- Class identity dựa trên tên + defining loader; không nhầm “cùng tên là cùng class”.
- `ClassNotFoundException` (lookup runtime) khác `NoClassDefFoundError` (link/init); đọc đúng signal.
- Thread Context ClassLoader có thể khác loader của class hiện tại; framework thường yêu cầu set rõ.
- Custom loader/redeploy phải release reference tới class metadata để tránh classloader leak.
- App business thông thường không nên tự viết classloader chỉ để học.
