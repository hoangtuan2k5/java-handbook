...
- Factory che giấu logic tạo object phức tạp; không che giấu type thông thường.
- Selection logic ưu tiên registry/lookup, không `switch` lớn rải rác.
- Không trả `null`; throw exception domain hoặc Optional rõ.
- Đặt tên factory theo intent business, không generic kiểu `ManagerFactory`.
- Cân nhắc cache instance khi tạo object nặng và immutable.
