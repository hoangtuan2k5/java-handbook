...
- Facade làm API đơn giản hơn cho subsystem; không nuốt error context.
- Tránh god facade; tách theo use case khi facade quá rộng.
- Facade không phải nơi đặt business logic chính; logic vẫn thuộc subsystem.
- Transaction boundary và side effect phải rõ trong API; không che mơ hồ.
- Nếu facade không đơn giản hóa caller, nó chỉ là tầng trung gian dư thừa.
